...
import "../DomManipulation/DomManipulation.scss";
...
export default function DomManipulation() {
  ...
  useEffect(() => {
    CATEGORIES_API.getAll("dom").then((res) => setFlipCard(res.data));
    // console.log(res);
  }, []);
  ...
  return (
    <section className="dom">
      <div className="dom__top">
        ...
        <div className="dom__count-container">
          <p className="dom__count-display">
            ...
            <span className="dom__number-modifier">
              {count >= 0 ? count : false}
            </span>
            /13
          </p>
        ...
        <div className="dom__button-divider">
          <div className="dom__floor-container">
            <span className="dom__floor-1">
              {" "}
              {("0" + Math.floor((timer / 60000) % 60)).slice(-2)}:
            </span>
            <span className="dom__floor-2">
              {" "}
              {("0" + Math.floor((timer / 1000) % 60)).slice(-2)}:
            </span>
            <span className="dom__floor-3">
              {" "}
      ...
      <div className="dom__card-divider">
        {flashcards.map((card) => {
          return (
            <>
              <div
                className={`dom__card-container ${
                  card.flipped ? "dom__flip-card" : ""
                } `}
              >
                <div
                  ...
                  className={`dom__card ${
                    card.correct
                      ? card.correct === "correct"
                        ? "dom__correct-background"
                        : "dom__incorrect-background"
                      : ""
                  }  ${
                    card.correct
                      ? card.correct === "clear"
              ...
              {card.flipped ? (
                <div className="dom__select-container">
                  <p
