...
export default function Es6() {
  ...
  useEffect(() => {
    CATEGORIES_API.getAll("es6").then((res) => setFlipCard(res.data));
    // console.log(res);
  }, []);
  ...
  return (
    <section className="es6">
      <div className="es6__top">
        ...
        <div className="es6__count-container">
          <p className="es6__count-display">
            ...
            <span className="es6__number-modifier">
              {count >= 0 ? count : false}
            </span>
            /20
          </p>
        ...
        <div className="es6__button-divider">
          <div className="es6__floor-container">
            <span className="es6__floor-1">
              {" "}
              {("0" + Math.floor((timer / 60000) % 60)).slice(-2)}:
            </span>
            <span className="es6__floor-2">
              {" "}
              {("0" + Math.floor((timer / 1000) % 60)).slice(-2)}:
            </span>
            <span className="es6__floor-3">
              {" "}
      ...
      <div className="es6__card-divider">
        {flashcards.map((card) => {
          return (
            <>
              <div
                className={`es6__card-container ${
                  card.flipped ? "es6__flip-card" : ""
                } `}
              >
                <div
                  ...
                  className={`es6__card ${
                    card.correct
                      ? card.correct === "correct"
                        ? "es6__correct-background"
                        : "es6__incorrect-background"
                      : ""
                  }  ${
                    card.correct
                      ? card.correct === "clear"
              ...
              {card.flipped ? (
                <div className="es6__select-container">
                  <p
